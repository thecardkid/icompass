...
module.exports.PROMPTS = {
    EXPORT: '<h2>I see you want to save this compass as a PDF. You can:</h2><p>1. Click \'Export\' and I can try to create a pdf for you, but the image quality may not be to your liking. Or</p><p>2. Take a screenshot (recommended)</p>',
    ...
    EXIT_DRAFT_WARNING: 'Changing mode now will discard all your drafts',
...
const getModal = (text, confirm, cancel, danger) => {
    return { text, confirm, cancel, danger };
};

module.exports.MODALS = {
    IMPORT_IMAGE: getModal(this.PROMPTS.CONFIRM_IMAGE_LINK, 'Import', 'Cancel', false),
    EXPORT_PDF: getModal(this.PROMPTS.EXPORT, 'Export', 'Cancel', false),
    SHARE_LINK: function(url) {
        return '<h2>Share this link below:</h2><p>' + url + '</p>';
    },

    DISCARD_DRAFT: getModal(this.PROMPTS.CONFIRM_DISCARD_DRAFT, 'OK', 'Cancel', true),
    EXIT_DRAFT_MODE: getModal(this.PROMPTS.EXIT_DRAFT_WARNING, 'Discard', 'Cancel', true),
    DELETE_NOTE: getModal(this.PROMPTS.CONFIRM_DELETE_NOTE, 'Delete', 'Cancel', true),
    BULK_DELETE_NOTES: getModal(this.PROMPTS.CONFIRM_BULK_DELETE_NOTES, 'Delete All', 'Cancel', true),
    DELETE_BOOKMARK: getModal(this.PROMPTS.CONFIRM_DELETE_BOOKMARK, 'Delete', 'Cancel', true),
    DELETE_COMPASS: getModal(this.PROMPTS.CONFIRM_DELETE_COMPASS, 'Delete', 'Cancel', true),
};
