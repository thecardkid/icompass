...
class WorkspaceSocket {
  constructor(session) {
    ...
    this.socket.on('create timer', this.createTimer.bind(this));
    this.socket.on('cancel timer', this.cancelTimer.bind(this));
  ...
  sendMessage(msg) {
    this.logMetric('Sent message');
    this.broadcast('new message', msg);
  }

  createTimer(min, sec, start) {
    this.logMetric('Create timer');
    this.broadcast('start timer', min, sec, start);
  }

  cancelTimer() {
    this.logMetric('Cancel timer');
    this.broadcast('all cancel timer');
  }
  ...
  findCompassEdit(data) {
    Compass.findByEditCode(data.code, (compass) => {
      if (compass !== null) {
        ...
        this.broadcast('user joined', { users: o.manager, joined: this.socket.username });
      }

      this.socket.emit('compass found', {
        compass: compass,
        username: this.socket.username || data.username,
        viewOnly: false,
      });
    });
  }
