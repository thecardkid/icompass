...
class WorkspaceSocket {
  ...
  sendMail(data) {
    ...
    Mail.sendMessage(text, data.email, 'Your iCompass workspace link', (status) => {
  ...
  sendMailBookmarks(data) {
    ...
    _.each(data.bookmarks, ({ center, href }) => {
      text += `${center}: ${HOST}${href}\n\n`;
    });

    Mail.sendMessage(text, data.email, 'Your iCompass bookmarks', (status) => {
      this.logMetric('Sent email of bookmarks');
      this.socket.emit('mail status', status);
    });
  }
  ...
  plusOneNote(id) {
    Compass.plusOneNote(this.compassId, id, (c) => {
      logger.info(this.username, '+1\'ed a note',id, 'in compass', this.compassId);
      this.broadcast('update notes', c.notes);
    });
  }
  ...
  bulkDragNotes(ids, { dx, dy }) {
    Compass.bulkDragNotes(this.compassId, ids, { dx, dy }, (c) => {
      this.broadcast('update notes', c.notes);
    });
  }
