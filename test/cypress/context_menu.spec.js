...
describe('context menus', () => {
  ...
  describe('shows at most one instance', () => {
    it('old one dismisses if a new one shows', () => {
      cy.get('body').dblclick(x, y);
      cy.get('#ic-form-text .ql-editor').type('text note');
      cy.get('button[name=ship]').click();
      cy.get('body').dblclick(x+400, y);
      cy.get('#ic-form-text .ql-editor').type('text note 2');
      cy.get('button[name=ship]').click();
      cy.get('div.ic-sticky-note').should('have.length', 2);

      cy.get('#note0').rightclick();
      cy.get('.context-menu').should('have.length', 1);
      cy.get('#note1').rightclick();
      cy.get('.context-menu').should('have.length', 1);
    });
  });


  describe('compass context menu', () => {
    it('works', () => {
      const left = x+400;
      const top = y+400;
      cy.get('body').rightclick(left, top);
      cy.get('.ic-menu-item').should('have.length', 1);
      cy.get('.ic-menu-item').click();
      cy.get('#ic-form-text .ql-editor').type('context menu note');
      cy.get('button[name=ship]').click();
      // Assert that the location of the note is the same as where the
      // context menu was invoked.
      cy.get('#note2').then($el => {
        const pos = $el.position();
        expect(pos.left).to.equal(left);
        expect(pos.top).to.equal(top);
      });
    });
  });
});
